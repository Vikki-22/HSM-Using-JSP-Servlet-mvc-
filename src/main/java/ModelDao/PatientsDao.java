...
// search Patient
	public List<Patients> Search(String name) {
		List<Patients> list = new ArrayList<>();
		try {
			Connection con = DBUtil.makeConnection();
			...
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
				Patients p = new Patients(rs.getInt("patient_id"), rs.getString("first_name"),
						rs.getString("last_name"), rs.getString("gender"), rs.getDate("date_of_birth"),
						rs.getString("phone"), rs.getString("email"), rs.getString("address"),
						rs.getString("blood_group"), rs.getDate("registration_date"));
				list.add(p);
			}
		} catch (Exception e) {
			e.printStackTrace();
