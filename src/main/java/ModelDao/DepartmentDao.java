...
public class DepartmentDao {
	...
	public List<Departments> viewDepartments() {
		...
		try {
			...
			while (rs.next()) {
				Departments d = new Departments(rs.getInt("department_id"), rs.getString("department_name"),
	...
	public Departments viewDepartmentById(int id) {
		...
		try {
			...
			ResultSet rs = pst.executeQuery();
			if (rs.next()) {
				d = new Departments(rs.getInt("department_id"), rs.getString("department_name"),
						rs.getString("location"));
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	//search
	public List<Departments> Search(String name) {
		List<Departments> list = new ArrayList<>();
		try {
			Connection con = DBUtil.makeConnection();
			...
			ResultSet rs = pst.executeQuery();
			while (rs.next()) {
				Departments d= new Departments(rs.getInt("department_id"), rs.getString("department_name"), rs.getString("location"));
				list.add(d);
			}
		} catch (Exception e) {
			e.printStackTrace();
